...
def _run_webhook_server(port: int):
    """Start Flask webhook server (for Render / cloud platforms)."""
    if not config.bot_token:
        print("ERROR: Bot token is required. Run --setup-webhook first.")
        sys.exit(1)
    print(f"Starting webhook server on port {port}")
    # Set webhook to Render's public URL (passed via --webhook-url or env)
    public_url = os.environ.get("RENDER_EXTERNAL_URL", "")
    webhook_url = None
    for arg in sys.argv:
        if arg.startswith("--webhook-url="):
            webhook_url = arg.split("=", 1)[1]
    webhook_url = webhook_url or public_url
    if webhook_url:
